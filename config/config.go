...
func parseNameServerPolicy(nsPolicy *orderedmap.OrderedMap[string, any], adapterName string, ruleProviders map[string]P.RuleProvider, respectRules bool, preferH3 bool) ([]dns.Policy, error) {
	...
	for pair := nsPolicy.Oldest(); pair != nil; pair = pair.Next() {
		...
		nameservers, err := dns.ParseNameServerWithOptions(servers, respectRules, preferH3)
		if err != nil {
			return nil, err
		}
...
func parseDNS(rawCfg *RawConfig, ruleProviders map[string]P.RuleProvider) (*DNS, error) {
	...
	var err error
	if dnsCfg.NameServer, err = dns.ParseNameServerWithOptions(cfg.NameServer, cfg.RespectRules, cfg.PreferH3); err != nil {
		return nil, err
	}

	if dnsCfg.Fallback, err = dns.ParseNameServerWithOptions(cfg.Fallback, cfg.RespectRules, cfg.PreferH3); err != nil {
		return nil, err
	}
	...
	if dnsCfg.ProxyServerNameserver, err = dns.ParseNameServerWithOptions(cfg.ProxyServerNameserver, false, cfg.PreferH3); err != nil {
		return nil, err
	}
	...
	if dnsCfg.DirectNameServer, err = dns.ParseNameServerWithOptions(cfg.DirectNameServer, false, cfg.PreferH3); err != nil {
		return nil, err
	}
	...
	if dnsCfg.DefaultNameserver, err = dns.ParseNameServerWithOptions(cfg.DefaultNameserver, false, cfg.PreferH3); err != nil {
		return nil, err
	}
