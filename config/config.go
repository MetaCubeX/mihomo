...
func ParseRawConfig(rawCfg *RawConfig) (*Config, error) {
	...
	for name, rules := range autoRulesMap {
		if _, exists := ruleProviders[name]; exists {
			log.Warnln("[AutoImportRules] rule-provider [%s] already exists, skipping auto-import", name)
			continue
		}
		ruleProviders[name] = RP.NewInlineProvider(name, P.Classical, rules, R.ParseRule)
		log.Infoln("[AutoImportRules] created inline rule-provider [%s] with %d rules", name, len(rules))
	}
...
func parseProxies(cfg *RawConfig) (proxies map[string]C.Proxy, providersMap map[string]P.ProxyProvider, err error) {
	...
	// First pass: collect auto-create-group info and generate proxy-group configs for providers
	autoGeneratedGroups := []map[string]any{}
	if err := provider.ParseProxyProviderForAutoGroupWithFilter(providersConfig, &autoGeneratedGroups, cfg.IPv6); err != nil {
		return nil, nil, fmt.Errorf("auto-create-group error: %w", err)
	}

	// Merge auto-generated groups into groupsConfig before proxyList building and DAG sort
	groupsConfig = append(groupsConfig, autoGeneratedGroups...)
	...
	log.Infoln("[AutoCreateGroup] After proxyGroupsDagSort, groupsConfig count: %d", len(groupsConfig))
	...
	log.Infoln("[AutoCreateGroup] AllProviders: %v", AllProviders)
