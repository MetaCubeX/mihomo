...
func TestTCPRedirect(t *testing.T) {
	...
	defer cancel()
	r := &tcpRedirect{nat: tun.NewNat(ctx, time.Minute), ports: [2]uint16{18474, 18475}, routes: make(map[uint16]address)}
	for _, ipv6 := range []bool{false, true} {
		...
		if addr, ok := r.redirect(p, info, address{IfIdx: 3, SubIfIdx: 4}); !ok || addr.Flags != flagOutbound {
			t.Fatal("TCP redirect failed")
		}
		redirected, ok := parsePacket(p)
		if !ok || redirected.destination != netip.AddrPortFrom(relayLocal(source.Addr()), r.port(source.Addr())) ||
			redirected.source.Addr() != relayPeer(source.Addr()) {
