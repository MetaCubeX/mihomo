...
const (
	// resolvingHostsKey is the context key for tracking hosts being resolved
	// This is used to detect DNS resolution loops
	resolvingHostsKey contextKey = iota
)
...
// checkDNSLoop checks if the host is already being resolved in the current context
// Returns an error if a loop is detected, or a new context with the host added
func checkDNSLoop(ctx context.Context, host string) (context.Context, error) {
	resolvingHosts, ok := ctx.Value(resolvingHostsKey).(map[string]struct{})
	if !ok {
		resolvingHosts = make(map[string]struct{})
	} else {
		// Check if this host is already being resolved
		if _, exists := resolvingHosts[host]; exists {
			return ctx, fmt.Errorf("%w: host %q is already being resolved", ErrDNSLoop, host)
		}
		// Make a copy to avoid modifying the original map
		newHosts := make(map[string]struct{}, len(resolvingHosts)+1)
		for h := range resolvingHosts {
			newHosts[h] = struct{}{}
		}
		resolvingHosts = newHosts
	}
	resolvingHosts[host] = struct{}{}
	return context.WithValue(ctx, resolvingHostsKey, resolvingHosts), nil
}
