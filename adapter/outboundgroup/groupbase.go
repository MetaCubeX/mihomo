...
func (gb *GroupBase) URLTest(ctx context.Context, url string, statusCodeRange []utils.Range[uint16]) (map[string]uint16, error) {
	...
	for _, proxy := range proxies {
		...
		wg.Add(1)
		go func() {
			delay, err := proxy.URLTest(ctx, url, statusCodeRange)
			if err == nil {
				lock.Lock()
				mp[proxy.Name()] = delay
				lock.Unlock()
