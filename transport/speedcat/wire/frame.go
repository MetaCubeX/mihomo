...
// FrameHeaderLen = type(1) + len(2) + ctr(4) = 7 字节(02 §3;对照 Rust frame::FRAME_HEADER_LEN)。
const FrameHeaderLen = 7

// AEADTagLen = ChaCha20-Poly1305 tag = 16 字节(03 §4 / 02 §3;对照 Rust crypto::TAG_LEN)。
const AEADTagLen = 16
...
// FrameHeader —— stream 帧头(02 §3):[type:u8][len:u16][ctr:u32],大端。
//
// Len:AEAD 路 = ciphertext+tag(payload+TAG);NO_INNER_AEAD(快路)= payload(无 tag)。
// Ctr:帧计数器,明文传输,接收方用它构造 nonce(base_nonce XOR be_bytes(ctr),03 §4.2)。篡改 ctr → nonce 错 → AEAD 解密失败 → 丢帧。
...
// BuildAAD 构造 AAD = [type][len:BE][ctr:BE](7 字节,02 §3)。type/len/ctr 全认证(纳入 AEAD AAD,防被改型/改长/改序)。
