...
// NO_INNER_AEAD(bit 8)快路自动置位(exporter 双方都在时,ADR-007);bit 9-15 reserved 须 0。
type Caps uint16

// 能力位旗(02 §1.2)。
const (
	CapHasDatagram Caps = 1 << 0 // HAS_DATAGRAM:传输层有原生 UDP datagram(仅 QUIC 置位);隐含承诺实现 §6.1 分片重组
...
// FEC_RATIO 占 bit 5-7(3 位枚举,02 §1.2 / 04 §4.1.1):0=off / 1=4:1 / 2=10:3 / 3=10:10。
const (
...
// SetFECRatio 设置 FEC 比率枚举(r 取低 3 位,02 §1.2)。
...
// 快路 exporter 双方在时自动置位 → 帧无 tag、payload 明文(ADR-007)。
...
// 指针 receiver(与 SetFECRatio 同款,原地改)。握手期 force 置/清(架构不变量 ADR-007:
...
// ServerHello caps_s / FastHello caps_c(全大端)。返回数组(栈分配,零逃逸 —— 热路径友好,ADR-005)。
...
// Negotiate 取握手能力交集(02 §1.2 有效交集规则)。
//
...
// handshake_secret → 伪装路会话密钥分叉(03 §3)。快路 auth_tag 绑 raw caps、不经 negotiate,不受影响。
