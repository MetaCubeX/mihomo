...
// FrameType —— 帧类型(02 §4;对照 Rust frame::FrameType)。纳入 AAD,防被改型(02 §3)。
type FrameType byte

const (
	FrameTCPConnect   FrameType = 0x01 // C→S:目标 Addr(02 §5)
	...
	FrameMuxOpen      FrameType = 0x0A // C→S:[flags:u16][destination:Addr](P2,§7)
)
...
// ErrorCode —— ERROR 帧(0x09)payload 错误码(02 §4)。
type ErrorCode byte
