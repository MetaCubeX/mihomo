...
// 仍加密应用数据(对端错 PSK → 双层 AEAD 解密失败 → 连接断)。**仅 dev/测试/受控网络**;生产伪装用 Reality
// (docs/05,真证书借身份层)。这条 arm 的存在是为 L4 伪装路跨实现 e2e 兜底(docs/17 §6 解死结)。
//
...
// fail-safe:仅 dev/测试/受控网络(无传输层加密;生产用 Reality)。
