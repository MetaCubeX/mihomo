...
//
// 设计/流程:对照 L2 / (传输 SSOT)/ (快路 exporter 命门)。
...
// (speedcat exporter 在 TLS 1.3 + EMS 下成立;TLS 1.2 无 EMS,exporter 语义不同)。
// TCP arm 用 stdlib crypto/tls(成熟);QUIC arm 用 metacubex/tls(API 兼容,quic-go fork 同款,
// 集成 mihomo 时 import path 零摩擦)。
//
// 冷热路径:dial/握手 = 冷路径(连接生命周期事件);热路径 relay 留 L4(零成本约束区不在此包)。
//
...
const (
	// KindTCP = TCP + TLS 1.3。
	KindTCP Kind = iota
	// KindQUIC = QUIC(metacubex/quic-go,multi-stream)。
	KindQUIC
...
// conn 实现此接口 + SupportsDatagrams() → datagram 臂(随机 nonce AEAD);否则 → 流隧道臂。
//
...
type DatagramConn interface {
	...
	// MaxDatagramPayloadSize 单 datagram 明文上限(含 header 预算;**已含 QUIC framing overhead**,
