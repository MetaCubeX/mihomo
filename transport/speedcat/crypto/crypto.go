...
// ChaCha20-Poly1305 AEAD / nonce 构造 / 会话密钥派生 / 快路 auth_tag(03 §2.1 快路 / §3 伪装路 / §4 AEAD+nonce)。
//
// 这是 L4 A2(docs/17 §3 五层 proto 库)的第 2 层:纯密码学,零网络,零 I/O。只依赖
// `metacubex/blake3`(mihomo transport/vless/encryption 同款,docs/19 §7-① 对齐)+ `golang.org/x/crypto`
// (chacha20poly1305 + curve25519 后续 handshake 用),都是 mihomo / sing 生态既有依赖(D3,docs/17 §0)。
//
// **设计铁律:与 Rust 端 `crates/proto-core/src/crypto.rs` 逐字节一致。** speedcat 是协议两份
// 实现(Rust 内核 + Go adapter,以 docs/02 SSOT 对照,见 docs/09 §5.5),密码学层任何字节分叉
// 都会让两端握手不上 —— 故本包每个函数都有 KAT(known-answer test),向量为 Rust 端实测印出后
// 冻进 `crypto_test.go`(Rust 当 oracle,跨实现一致性铁证,见 docs/17 §6 承重风险)。
//
// **承重风险已解(docs/17 §6):** BLAKE3 有两种工作模式,Rust 用 `blake3::derive_key`(KDF 模式,
...
//
// **API 形不同语义同(docs/19 §7-① 承重):** metacubex/blake3 v0.1.0 早于 zeebo/blake3 v0.2.4 的
...
//
// **冷热路径边界(ADR-005):** 本包是热路径(relay/AEAD/帧)的底层。AEAD seal/open 各 1 次 alloc
// (与 Rust 端 Session 同成本),MVP 先正确,原地零拷贝留收尾(同 Rust,04 §「先正确后零拷贝」)。
...
const (
	...
	HsNonceLen = 16 // 伪装路 nonce_c / nonce_s 随机(02 §2 ClientHello/ServerHello)
...
// 密钥派生 context 字符串(03 §3,域分离)。与 Rust crypto.rs:193-196 逐字符一致 ——
...
const (
	...
	HSCtx         = "speedcat-v1-hs"        // 伪装路握手 MAC 输入前缀(03 §3)
	AuthKeyCtx    = "speedcat-v1 auth key"  // 快路 k_auth = DeriveKey(this, psk)
	FastAuthMsg   = "speedcat-v1 fast-auth" // 快路 auth_tag MAC 明文前缀(03 §2.1)
	ExporterLabel = "speedcat-v1 exporter"  // TLS exporter label(handshake.rs:16,03 §2.1)
	// UDPExporterLabel UDP datagram 独立 AEAD 上下文(DatagramCipher,ADR-009)的 exporter label。
...
// ② 值语义,跨 goroutine 复制零共享(热路径无锁,ADR-005)。
type (
	Key [KeyLen]byte
	Psk [KeyLen]byte
)
...
// 非 hex 字符 → 错误(fail-loud,§4.4)。
//
// 注意:凭据管理铁律(§6.3)—— PSK 经此解析后进内存;调用方勿把明文 PSK 进日志(§5.4)。
