// datagram_cipher.go —— UDP datagram 独立 AEAD 上下文(镜像 Rust datagram.rs:39-89;ADR-009)。
//
...
// 故 UDP 走**独立** AEAD 上下文 DatagramCipher(ADR-009)。
//
// # 设计(ADR-009)
...
//     零协调(热路径无锁,ADR-005)。碰撞:birthday bound ~2⁴⁸ datagrams,远超连接生命周期。
//   - 不加重放保护:UDP 网络层本就可被重放;应用层重放(DNS txn id / QUIC initial)归应用,隧道不加第二层。
//
// # 线格式(单 datagram payload)
//
//	[nonce:12][AEAD_ct(§6.1 header + frag_payload)]
//
// nonce 明文前置(接收方需 nonce 才能解密);§6.1 header 与 frag_payload 均在密文内(ASSOC_ID 也加密)。
// AAD = 空。
//
// **热路径(ADR-005):** Seal/Open 各 1 次 alloc(同 Session AEAD 成本);MVP 先正确,零拷贝留收尾。**禁每 datagram 日志**。
// **panic-free**(被 mihomo import 的库:AEAD/随机错返 error)。
//
// §5.4:导出密钥/key 是密钥 —— self-test 只 bytes.Equal / errors.Is,**绝不打 raw**(对照 Rust datagram.rs 测试)。
...
// DatagramCipher UDP datagram 独立 AEAD 上下文(随机 nonce、无 ctr、无重放窗口;ADR-009)。
...
// exporter 不可用 → fail-loud(凭据安全铁律 §4.4:不静默退到无加密)。
