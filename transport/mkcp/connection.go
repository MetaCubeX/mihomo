...
func NewConnection(meta ConnMetadata, writer PacketWriter, closer io.Closer, config *Config) *Connection {
	log.Debugln("[mkcp] #%d creating connection to %s", meta.Conversation, meta.RemoteAddr)
...
// ReadMultiBuffer reads queued data as a MultiBuffer.
...
// WriteMultiBuffer writes a MultiBuffer to the connection.
...
func (c *Connection) SetState(state State) {
	...
	log.Debugln("[mkcp] #%d entering state %d at %d", c.meta.Conversation, state, current)
...
func (c *Connection) Close() error {
	...
	log.Debugln("[mkcp] #%d closing connection to %s", c.meta.Conversation, c.meta.RemoteAddr)
...
// SetDeadline sets the read and write deadlines. A zero value disables them.
...
func (c *Connection) Terminate() {
	...
	log.Debugln("[mkcp] #%d terminating connection to %s", c.meta.Conversation, c.RemoteAddr())
...
func (c *Connection) flush() {
	...
	if c.State() == StateTerminating {
		log.Debugln("[mkcp] #%d sending terminating cmd", c.meta.Conversation)
